   ...
   const renderSeparator=()=>{
    ...
    return(
        <View style={{backgroundColor:colors}}>
            <Text style={styles.headtxt}>{'Comming Soon'}</Text>
            {/* <Text style = {styles.headtxt}>{movie[0].day+' ('+Moment(movie[0].button_url).format('DD-MM-YYYY')+')'}</Text> */}
       <FlatList
        style={{marginTop:5,borderRadius: 6,backgroundColor:colors.white,marginBottom:23}}
        data={movie}
        ...
        renderItem={({ item }) => (
         ...
         <View style={styles.rowitems}>
            ...
            <View style = { styles.moviedetail }>
                 <Text style={styles.rowtitle}>{item.text_box}</Text>
                {/* {_btnVerify(item)} */}
            </View>
         </View>
       </View> 
